...
export function DashboardShell({ children, active }: { children: ReactNode; active: string }) {
  ...
  return <main className="min-h-screen bg-white/35 backdrop-blur-xl text-[#16231e] md:grid md:grid-cols-[236px_1fr]">
    <aside className="hidden min-h-screen flex-col bg-[#163c30]/85 backdrop-blur-xl px-5 py-8 text-[#eef5e9] md:flex">
...
export function EmptyWorkspaceCard() {
  return <DashboardShell active="Overview">
    <section className="grid max-w-3xl gap-6 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-8">
      <div>
...
export function ConfigErrorCard({ message }: { message: string }) {
  return <main className="min-h-screen bg-white/35 backdrop-blur-xl px-5 py-10 text-[#16231e]">
    <section className="mx-auto max-w-3xl rounded-2xl border border-red-200/70 bg-white/70 backdrop-blur-xl p-8">
      ...
      <p className="mt-4 rounded-lg bg-white/35 backdrop-blur-xl p-4 font-mono text-xs">Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in .env.local and in your deployment environment.</p>
    </section>
  </main>;
}