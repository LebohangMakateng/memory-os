...
export default async function DashboardPage() {
  ...
  return <DashboardShell active="Overview">
    ...
    <section className="mb-6 rounded-2xl bg-[#d8ef61]/75 backdrop-blur-xl p-7">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.6fr_.9fr]">
      <article className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          {next ? <span className="rounded-full bg-white/45 backdrop-blur-xl px-3 py-1 text-xs font-bold">{next.task.estimateMinutes} min</span> : null}
        </div>
        {next ? <>
          ...
          <div className="mt-5 border-l-[3px] border-[#f0aa72] bg-white/35 backdrop-blur-xl p-4">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
            <span className="rounded bg-[#d8ef61]/25 backdrop-blur-xl px-2 py-1 text-xs font-bold capitalize text-[#3d6729]">{next.task.focusType} focus</span>
      ...
      <article className="flex min-h-56 flex-col rounded-2xl bg-[#163c30]/85 backdrop-blur-xl p-7 text-white">
        ...
        <div className="mt-6 h-1.5 rounded bg-white/20"><div className="h-1.5 rounded bg-[#d8ef61]/75 backdrop-blur-xl" style={{ width: `${completionPercent}%` }} /></div>
    ...
    <section className="mt-6 grid gap-6 md:grid-cols-3">
      <a className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-5" href="/dashboard/projects"><p className="text-3xl font-bold">{overview.projects.length}</p><p className="mt-1 text-sm font-bold">Active projects</p><p className="mt-2 text-xs text-[#64726b]">{activeProject?.title ?? "Create a project"}</p></a>
      <a className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-5" href="/dashboard/opportunities"><p className="text-3xl font-bold">{overview.opportunities.length}</p><p className="mt-1 text-sm font-bold">Inbox ideas</p><p className="mt-2 text-xs text-[#64726b]">Capture without switching focus.</p></a>
      <a className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-5" href="/dashboard/planning"><p className="text-3xl font-bold">{overview.planningDrafts.length}</p><p className="mt-1 text-sm font-bold">Pending drafts</p><p className="mt-2 text-xs text-[#64726b]">Review before approval.</p></a>
    </section>
  </DashboardShell>;
}