...
export function FinanceAdvancedWorkspace(props: Props) {
  ...
  return <section className="mt-6 grid gap-6">
    {(error || saved) ? <div className="rounded-lg border border-[#cad5cb]/70 bg-white/70 backdrop-blur-xl px-4 py-3 text-sm font-bold">{saved ? <p className="text-[#163c30]">{saved}</p> : null}{error ? <p className="text-red-700">{error}</p> : null}</div> : null}

    <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      <article className="scroll-mt-28 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-analytics">
        ...
        <div className="mt-5 grid gap-3">
          {props.analytics.slice(0, 8).map((row) => <div className="rounded-xl bg-white/35 backdrop-blur-xl p-4" key={row.category.id}>
      ...
      <article className="scroll-mt-28 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-forecast">
        ...
        <div className="mt-5 rounded-xl bg-white/35 backdrop-blur-xl p-4 text-sm leading-6 text-[#64726b]">{props.review.forecast}</div>
      </article>
    </section>

    <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
      <details className="mobile-disclosure"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">PDF extraction</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><form className="scroll-mt-28 grid gap-4 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-pdf" onSubmit={uploadPdf}>
      ...
      <details className="mobile-disclosure"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">Paste statement text</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><form className="scroll-mt-28 grid gap-4 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-statements" onSubmit={previewStatement}>
      ...
      <article className="scroll-mt-28 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-review">
        ...
        <div className="mt-5 grid gap-3 text-sm leading-6 text-[#64726b]">
          {props.review.biggestMoneyLeaks.length ? props.review.biggestMoneyLeaks.map((item) => <p className="rounded-xl bg-white/35 backdrop-blur-xl p-3" key={item}>{item}</p>) : <p className="rounded-xl bg-white/35 backdrop-blur-xl p-3">No obvious money leaks yet.</p>}
          {props.review.priorities.map((item) => <p key={item}>Priority: {item}</p>)}
        </div>
      </article>
    </section>

    {previewRows.length ? <article className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6">
      ...
      <div className="mt-5 grid gap-3">
        {previewRows.map((row, index) => <div className="grid gap-2 rounded-xl bg-white/35 backdrop-blur-xl p-4 md:grid-cols-[80px_1fr_120px_150px_90px]" key={row.id}>
    ...
    <section className="grid gap-6 xl:grid-cols-3">
      <details className="mobile-disclosure xl:col-span-3"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">Scenarios and check-ins</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><article className="scroll-mt-28 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6 xl:col-span-3" id="finance-planning-tools">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <div className="grid gap-3"><button className={secondaryButton} onClick={loadComparison} type="button">Historical comparison</button><pre className="max-h-48 overflow-auto rounded-lg bg-white/35 backdrop-blur-xl p-3 text-xs">{comparisonResult || "No comparison loaded."}</pre></div>
          <div className="grid gap-3"><button className={secondaryButton} onClick={loadCheckIn} type="button">Weekly check-in</button><pre className="max-h-48 overflow-auto rounded-lg bg-white/35 backdrop-blur-xl p-3 text-xs">{checkInResult || "No check-in loaded."}</pre></div>
          <form className="grid gap-3" onSubmit={runScenarioForm}><input className={inputClass} name="incomeDelta" placeholder="Income change R" /><input className={inputClass} name="expenseDelta" placeholder="Expense increase/cut R" /><input className={inputClass} name="savingsDelta" placeholder="Savings increase R" /><button className={secondaryButton} type="submit">Run scenario</button><pre className="max-h-48 overflow-auto rounded-lg bg-white/35 backdrop-blur-xl p-3 text-xs">{scenarioResult || "No scenario run."}</pre></form>
        </div>
      </article></div></details>
      <details className="mobile-disclosure"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">Savings goal</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><div className="scroll-mt-28 contents" id="finance-goals"><CreateCard title="Savings goal" onSubmit={(event) => postForm(event, "/api/finance/goals", (data) => ({ name: data.get("name"), goalType: data.get("goalType"), targetAmount: data.get("targetAmount"), currentAmount: data.get("currentAmount") || "0.00", targetDate: data.get("targetDate") || null, monthlyContribution: data.get("monthlyContribution") || "0.00", priority: Number(data.get("priority") || 3) }), "Goal saved.") }>
      ...
      </CreateCard></div></details>

      <details className="mobile-disclosure"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">Commitment</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><CreateCard title="Commitment" onSubmit={(event) => postForm(event, "/api/finance/commitments", (data) => ({ name: data.get("name"), amount: data.get("amount"), dueDate: data.get("dueDate"), categoryId: data.get("categoryId") || null, notes: data.get("notes") || null }), "Commitment saved.") }>
        <label className={labelClass}>Name<input className={inputClass} name="name" required /></label>
        <label className={labelClass}>Amount (R)<input className={inputClass} name="amount" required /></label>
    ...
    <section className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
      <details className="mobile-disclosure xl:col-span-2"><summary className="rounded-2xl border border-[#dce4dd]/70 bg-white/70 p-4 text-sm font-bold text-[#163c30] shadow-sm backdrop-blur-xl">Preferences</summary><div className="mobile-disclosure-content mt-3 md:mt-0"><form className="scroll-mt-28 grid gap-4 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6 xl:col-span-2" id="finance-settings" onSubmit={savePreferences}>
      ...
      <article className="scroll-mt-28 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" id="finance-records">
...
function CreateCard({ title, children, onSubmit }: { title: string; children: React.ReactNode; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <form className="grid gap-4 rounded-2xl border border-[#dce4dd]/70 bg-white/70 backdrop-blur-xl p-6" onSubmit={onSubmit}>
...
function ManageList({ title, rows }: { title: string; rows: { id: string; label: string; edit: () => void; del: () => void }[] }) {
  return <div className="rounded-xl bg-white/35 backdrop-blur-xl p-4"><p className="text-sm font-bold text-[#16231e]">{title}</p><div className="mt-3 grid gap-3 text-xs leading-5 text-[#64726b]">{rows.length ? rows.map((row) => <div className="grid gap-2" key={row.id}><p>{row.label}</p><div className="flex gap-2"><button className={secondaryButton} onClick={row.edit} type="button">Edit</button><button className={secondaryButton} onClick={row.del} type="button">Delete</button></div></div>) : <p>None recorded.</p>}</div></div>;
}
