...
import { FormEvent, TextareaHTMLAttributes, useEffect, useMemo, useRef, useState, useTransition } from "react";
...
function AutoTextarea(props: TextareaHTMLAttributes<HTMLTextAreaElement>) {
  ...
  return <textarea {...props} ref={ref} rows={1} onInput={(event) => {
    resize();
    props.onInput?.(event);
  }} />;
}
...
function TaskFields({ milestones, priorities, task }: { milestones: Milestone[]; priorities: Priority[]; task?: Task }) {
  return <>
    ...
    <label className={labelClass}>Description<AutoTextarea className={textAreaClass} defaultValue={task?.description ?? ""} name="description" /></label>
    <label className={labelClass}>Next action<AutoTextarea className={textAreaClass} defaultValue={task?.nextAction} name="nextAction" required /></label>
    <div className="grid gap-3 sm:grid-cols-2">
      <label className={labelClass}>Reason<AutoTextarea className={textAreaClass} defaultValue={task?.reason} name="reason" required /></label>
      <label className={labelClass}>Expected outcome<AutoTextarea className={textAreaClass} defaultValue={task?.expectedOutcome} name="expectedOutcome" required /></label>
    </div>
        ...
        {tasks.length ? tasks.map((task) => {
          ...
          return <article className="rounded-xl border border-[#dce4dd] bg-[#f5f7f2] p-4" key={task.id}>
            {editingId === task.id ? <form className="grid gap-3" onSubmit={(event) => updateTask(event, task.id)}>
              ...
              {task.description ? <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#4b5f55]">{task.description}</p> : null}
