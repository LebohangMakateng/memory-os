...
import { parseMoneyToCents } from "@/lib/finance/calculations";
import { runScenario } from "@/lib/finance/intelligence";
import { financeScenarioSchema } from "@/lib/finance/validation";
...
export async function POST(request: Request) {
  ...
  const parsed = financeScenarioSchema.safeParse(await request.json());
  if (!parsed.success) return Response.json({ error: parsed.error.flatten() }, { status: 400 });
  try {
    const scope = await loadFinanceScope();
    return Response.json(runScenario({ ...scope, incomeDeltaCents: parseMoneyToCents(parsed.data.incomeDelta), expenseDeltaCents: parseMoneyToCents(parsed.data.expenseDelta), savingsDeltaCents: parseMoneyToCents(parsed.data.savingsDelta) }));
  } catch (caught) {
    return Response.json({ error: caught instanceof Error ? caught.message : "Could not run scenario." }, { status: 500 });
  }
}
