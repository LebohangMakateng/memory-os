...
import { weeklyCheckIn } from "@/lib/finance/intelligence";
...
export async function GET() {
  ...
  try {
    ...
    return Response.json(weeklyCheckIn(scope));
  } catch (caught) {
    return Response.json({ error: caught instanceof Error ? caught.message : "Could not build weekly check-in." }, { status: 500 });
  }
}
